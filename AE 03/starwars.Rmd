...
**How many observations (rows) are there?** 87 observations

**How many variables (columns) are there?** 14 variables
...
Each observation is a set of several categorical and numerical characteristics for each Star Wars Character in the dataset.
...
The 'color = gender' mapping is not showing up on the graph. Instead, all of the data points are pink.
...
I think the 'color = gender' mapping was overridden by the 'color = pink' statement inside the geom_point() argument. Making the color of the data pink inside the geom_point function makes all of the points pink, regardless of what was written before.
...
ggplot(starwars, 
  ...
  labs(
    title = "Correlation Between Height and Mass of Star Wars Characters",           # Title about relationship between height and mass
...
There is no 'color' label in the labs() function because the color of each point has already been determined by the geom_point function.
...
I chose a binwidth of 20 because it allows me to see 9 bins while a binwidth of 50 only allows me to see 5 bins. I think a binwidth of 20 allows me to better see the shape of the distribution and which values the mass is concentrated around for all characters.

**Task 4.2:** Describe the distribution. Is it symmetric, skewed left, or skewed right? Are there any outliers?

**Your answer:**

The distribution is skewed right. Most masses of the Star Wars characters are around ~125 kg while there are a few observations with masses beyond ~125kg.
...
ggplot(starwars, aes(x = gender, y = height)) +
  ...
  labs(
    title = "The height of different genders in the Star Wars Universe",
    x = "Gender",
    y = "Height (cm)"
  )
```
...
The visualization tells me that masculine characters tend have a higher median height than other characters (feminine, NA). I notice that feminine characters tend to be the shortest (lowest median height), while characters that do not have a gender tend to lie somewhere in the middle.  
...
ggplot(starwars, aes(x = eye_color)) +
  geom_bar(fill = "coral") +
  labs(
    title = "Distribution of eye colors in the Star Wars Universe",
    x = "Eye color",
    y = "frequency"
  )
```
...
**Your answer:** The brown eye color category has the most observations.
...
```{r cat-cat-stacked}
# Your code here
ggplot(starwars, aes(x = hair_color, fill = gender)) +
  geom_bar() +
  labs(
    title = "Distribution of hair colors with respect to gender",
    x = "Hair Color",
    y = "Count",
    fill = "Gender"
  ) +
...
My stacked bar-plot visualization tells me that most hair colors are primarily portrayed by masculine characters, with the exception of blonde hair and auburn white hair and auburn grey hair, which are represented by only feminine characters.
...
starwars_filtered <- starwars %>%
  filter(species %in% c("Human", "Droid", "Hutt"))  # Pick 3-4 species
...
ggplot(starwars_filtered, 
  ...
  facet_wrap(~ species) +
  labs(
    title = "Heights and Masses of Star Wars Characters across genders and species",
    x = "Height (cm)",
    y = "Mass (kg)",
    color = "Gender"
  )
...
I notice that the droid characters in the dataset are all masculine. I also notice that there is only one observation of a Hutt species character who has a very high mass compared to the rest of the dataset in my chart. I notice across the facets that as height increases, mass also increases. They have a positive correlation.
...
Personally, I enjoyed using the histogram the most to explore the data because it was very readable and easier to identify patterns in the dataset, but it sacrificed detail. In comparison, the box plots and dot plots were more detailed. The box plot also provided information such as median and quartile ranges. 

**Task 9.2:** What was the most interesting pattern or relationship you found?

**Your answer:**

The most interesting pattern I found was how yellow and orange eye colors were relatively common in the Star Wars Universe. I was expecting green to be relatively common but on the histogram, there was only a 1-2 observations.

**Task 9.3:** What did you find most challenging about creating these visualizations?

**Your answer:**

I found plotting multiple variables in the same plot in Exercise 8 to be the most challenging, especially on a dot plot. I have not done this before, so it was interesting to see how multiple variables can be visualized in the same plot.
